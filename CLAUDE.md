...
- **动画栏**: 页面底部居中胶囊条（默认收起），列出所有动画（含 idle/action）。左侧 `>` 按钮展开/收起。收起时点击 canvas 播放 action（一次性回 idle）。展开时 canvas 点击禁用，点击 chip 循环播放并蓝色高亮。收起时自动回 idle + 清除高亮。鼠标滚轮/拖动滚动
...
| data-viewport | padded（正方形居中+20% margin）、centered（保持比例居中+20% margin）或 raw（精确包围盒） |
